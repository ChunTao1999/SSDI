...
from utils.utils import corrupt_img_landmark_shuffle, corrupt_img_permute, corrupt_img_black_box, corrupt_img_gaussian_blurring
# Debug
import pdb
...
class SUNRGBD(Dataset):
    def __init__(self, transform=None, data_dir=None, phase_train=True, all_corrupt=False, corruption_type="", num_distortion=1, num_permute=3, num_box=1, ps=160):

        self.phase_train = phase_train
        self.transform = transform
        self.all_corrupt = all_corrupt
        self.corruption_type = corruption_type # choose one among ["patch_shuffling", "puzzle_solving", "black_box", "gaussian_blurring"]
        self.num_distortion = num_distortion # used in "patch_shuffling"
        self.num_permute = num_permute # used in "puzzle_sovling"
        self.num_box = num_box # # used in "black_box" and "gaussian_blurring"
        self.ps = ps
        ...
        # 4.25.2023 - tao88: add corrupt labels for test images (0 and 1 array of len)
        if not self.phase_train:
            self.corrupt_labels = torch.zeros(len(self.img_dir_test))
            corrupt_indices = random.sample(range(len(self.img_dir_test)), len(self.img_dir_test) // 2)
            self.corrupt_labels[corrupt_indices] = 1 # half of corruptness labels flipped
    ...
    def __getitem__(self, idx):
        ...
        label = np.load(label_dir[idx]) # use 808 for demo
        ...
        # If during test, check the corrupt_label to determine if need to corrupt the image
        if not self.phase_train:
            sample['depth'] = sample['depth'].squeeze()
            # sample['corrupt_lbl'] = 1
            sample['corrupt_lbl'] = self.corrupt_labels[idx]
            if sample['corrupt_lbl'] == 1 or self.all_corrupt:
                if self.corruption_type == "patch_shuffling":
                    sample['image'], sample['depth'] = corrupt_img_landmark_shuffle(tensors=[sample['image'], sample['depth']], 
                                                                                    num_distortion=self.num_distortion, 
                                                                                    patch_size=self.ps)
                elif self.corruption_type == "puzzle_solving":
                    sample['image'], sample['depth'] = corrupt_img_permute(tensors=[sample['image'], sample['depth']], 
                                                                           num_permute=self.num_permute, 
                                                                           patch_size=self.ps)
                elif self.corruption_type == "black_box":
                    sample['image'], sample['depth'] = corrupt_img_black_box(tensors=[sample['image'], sample['depth']],
                                                                             num_box=self.num_box,
                                                                             patch_size=self.ps)
                elif self.corruption_type == "gaussian_blurring":
                    sample['image'], sample['depth'] = corrupt_img_gaussian_blurring(tensors=[sample['image'], sample['depth']],
                                                                                     num_box=self.num_box,
                                                                                     patch_size=self.ps,
                                                                                     kernel_size=(31, 31),
                                                                                     sigma=10)
