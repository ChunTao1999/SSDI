...
model_FPN, optimizer, classifier1 = get_model_and_optimizer(args, logger)
...
with torch.no_grad():
    for batch_idx, (index, images, targets) in enumerate(train_loader): # target_type=['attr', 'bbox', 'pred_init_glimpse_locs', 'landmarks', 'masks']
        translated_images, targets, bbox_targets, aligned_masks = images.to(device), targets[0].float().to(device), targets[1].to(device), targets[4].to(device)
        # translated images (128, 3, 256, 256); targets (128, 40); bbox_targets (128, 4)

        # 2.8.2023 - tao88
        # bboxes are resized together with original images, we can use them to crop out the aligned face image, and pass to the Resnet
        # visualize images and aligned masks
        # save_image(translated_images[:16], '/home/nano01/a/tao88/2.8/wild.png')
        # save_image(aligned_masks[:16], '/home/nano01/a/tao88/2.8/aligned_masks.png')
        # pdb.set_trace()

        # 2.8.2023 - tao88
        # now rescale the aligned masks, according to the bbox width and height, use NEAREST_EXACT for interpolation
        # pdb.set_trace()
        for img_idx in range(translated_images.shape[0]):
            x, y, w, h = int(bbox_targets[img_idx][0].round()), \
                        int(bbox_targets[img_idx][1].round()), \
                        int(bbox_targets[img_idx][2].round()), \
                        int(bbox_targets[img_idx][3].round())
            if w == 0: w = 1 # prevent zero h & w
            if h == 0: h = 1
            mask = aligned_masks[img_idx]
            resized_mask = F.resize(mask, 
                                    (h, w),
                                    interpolation=PIL.Image.NEAREST) # downscale the mask, possibly loss of information
            new_mask = torch.zeros_like(translated_images[img_idx])
            new_mask[:, y:y+h, x:x+w] = resized_mask
            # store
            save_image(new_mask, "/home/nano01/a/tao88/celebA_raw/Labels/labels_trainval_wild/{}.jpg".format(index[img_idx]))
            # visualize
            # save_image(translated_images[img_idx], '/home/nano01/a/tao88/2.9/orig_img_{}.png'.format(img_idx))
            # save_image(new_mask, '/home/nano01/a/tao88/2.9/new_mask_{}.png'.format(img_idx))
        ...
        # save_image((new_lbl[:16]).unsqueeze(1), '/home/nano01/a/tao88/2.7/aligned_masks.png')
        # pdb.set_trace()

        # Saving to designated folder
        # for i in range(new_lbl.shape[0]):
        #     save_image(new_lbl[i], '/home/nano01/a/tao88/celebA_raw/Labels/labels_face_parts_trainval_aligned_new/{}.jpg'.format(index[i])) # remember to +1 to match the image indices
        # if (batch_idx+1) % 100 == 0:
        #     print("Processed: {}/{}".format(batch_idx + 1, len(train_loader)))
