...
# tao88
import pdb
from utils_custom_tvision_functions import *
...
def get_dataset(args, mode, inv_list=[], eqv_list=[]):
    ...
    # tao88 - insert the option for creating celeba datasets
    elif args.celeba:
        if mode == 'train':
            # need to add inv_list, eqv_list, and scale to the dataset class
            dataset = CelebA_inthewild(args.data_root, labeldir=args.save_model_path, mode='compute', split = 'train', target_type=['attr', 'bbox', 'pred_init_glimpse_locs'],
                                       transform                        = custom_Compose([transforms.Resize(size=args.full_res_img_size)]),
                                                                        #   custom_RandomHorizontalFlip(p=0.5),
                                                                        #   transforms.ToTensor()]),  # use transform_tensor in dataloader_celebA_inthewild
                                       download=False,
                                       selected_attributes              = args.selected_attributes,
                                       at_least_true_attributes         = args.at_least_true_attributes, 
                                       treat_attributes_as_classes      = args.treat_attributes_as_classes,
                                       landmark_shuffle                 = args.landmark_shuffle,
                                       res1=args.res1,
                                       res2=args.res2,
                                       inv_list=inv_list, 
                                       eqv_list=eqv_list,
                                       scale=(args.min_scale, 1))
            if args.correct_imbalance:
                cnt_pos_attr = dataset.all_attr.sum(dim=0)
                cnt_neg_attr = dataset.all_attr.shape[0] - cnt_pos_attr
                ...
                equal_weights = torch.tensor([1.0 for _ in range(args.num_classes)])
                return dataset, equal_weights
        # elif mode == 'train_val':
        #     dataset = CelebA_inthewild(args.data_root, split = 'valid', target_type=['attr', 'bbox', 'pred_init_glimpse_locs'], 
        #                                transform                        = custom_Compose([transforms.Resize(size=args.full_res_img_size),
        #                                                                   transforms.ToTensor()]), 
        #                                selected_attributes              = args.selected_attributes,
        #                                at_least_true_attributes         = args.at_least_true_attributes, 
        #                                treat_attributes_as_classes      = args.treat_attributes_as_classes,
        #                                landmark_shuffle                 = args.landmark_shuffle)
        elif mode == "prepare_mask":
            dataset = CelebA_inthewild(args.data_root, labeldir=args.save_model_path, mode='prepare_mask', split = 'trainval', target_type=['attr', 'bbox', 'pred_init_glimpse_locs'],
                                        transform                        = custom_Compose([transforms.Resize(size=args.full_res_img_size),
                                        #                                     #   custom_RandomHorizontalFlip(p=0.5),
                                                                                    transforms.ToTensor()]),
