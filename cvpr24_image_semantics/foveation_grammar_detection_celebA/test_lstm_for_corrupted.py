            ...
            for img_id in range(16):
                if not config_3.landmark_shuffle:
                    if not config_3.use_corrupted_testset:
                        save_path = '/home/nano01/a/tao88/3.8_LSTM/correct/img_{}'.format(img_id)
                    else:
                        save_path = '/home/nano01/a/tao88/3.8_LSTM/corrupted_testset/img_{}'.format(img_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.8_LSTM/corrupted/img_{}'.format(img_id)
...
# =============================================================================
        ...
        for g in range(config_3.num_glimpses):
            ...
            if g == 0:
                ...
                for sample_id in range(16):
                    dist_16_imgs[sample_id].append((output_lstm[sample_id]-g_semantics_pred[sample_id]).norm().item())
        ...
        for sample_id in range(16):
            if not config_3.landmark_shuffle:
                if not config_3.use_corrupted_testset:
                    save_path = '/home/nano01/a/tao88/3.8_LSTM/correct/img_{}'.format(sample_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.8_LSTM/corrupted_testset/img_{}'.format(sample_id)
            else:
                save_path = '/home/nano01/a/tao88/3.8_LSTM/corrupted/img_{}'.format(sample_id)
            plot_curve(np.arange(1, 16), dist_16_imgs[sample_id], 'Dist btw. prediction and glimpse semantics', 'Glimpse iteration', 'L2 distance', os.path.join(save_path, 'plot_dist.png'))
        pdb.set_trace()
