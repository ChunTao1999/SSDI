...
save_path = './lstm_models_crop_semantics_lr={}'.format(config_4.lr_start)
...
for epoch in range(0, config_3.epochs):
    ...
    for i, (indices, images, targets) in enumerate(train_loader):
        ...
        mask_semantics = torch.nn.functional.one_hot(masks_extracted_resized).sum(dim=2) # (128, 5, 7)
        mask_semantics_ratio = (mask_semantics.float()) / num_pixels # (128, 5, 7) (N, L, H_in)

        # Initialize the LSTM before glimpse iteration
        optimizer_M4.zero_grad()
        h0, c0 = model_4._init_hidden(translated_images.shape[0]) 
        # (1, 128, 7) (D*num_layers, N, H_out), (1, 128, 7) (D*num_layers, N, H_cell)
        loss_lstm = torch.tensor([0.0]).cuda()

        # Pass the mask semantics as input to the LSTM. Since here we already have all mask semantics in the sequence, we can pass in one-shot (don't need to iterate)
        output_lstm, (hn, cn) = model_4(mask_semantics_ratio, (h0, c0))

        # compute MSE/cross-entropy loss between output_lstm[:-1] and mask_semantics_ratio[1:]
        loss_lstm = mse_loss(output_lstm[:, :-1, :], mask_semantics_ratio[:, 1:, :]) # both (128, 4, 7)
