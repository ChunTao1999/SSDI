...
def get_model_and_optimizer(args, logger):
    ...
    model = nn.DataParallel(model) # comment for same device
    model = model.cuda()
    ...
    # tao88 - 1.15, add option to only load the finetuned feature extractor model, need to set both restart and eval_only to false
    # if args.model_finetuned:
    #     load_path = args.finetuned_model_path
    #     # load_path = os.path.join(args.save_model_path, 'checkpoint_20_epochs.pth.tar')
    #     if os.path.isfile(load_path):
    #         checkpoint  = torch.load(load_path)
    #         args.start_epoch = checkpoint['epoch']
    #         model.load_state_dict(checkpoint['state_dict']) # we don't want the classifier param so that we can cluster to arbituary num_clusters
    #         # classifier.load_state_dict(checkpoint['classifier1_state_dict'])
    #         logger.info('Loaded finetuned model checkpoint. [epoch {}]'.format(args.start_epoch))
    #     else:
    ...
    if args.restart or args.eval_only: 
        ...
        if os.path.isfile(load_path):
            ...
            # new_state_dict['classifier.weight'] = checkpoint['classifier1_state_dict']['module.weight']
            # new_state_dict['classifier.bias'] = checkpoint['classifier1_state_dict']['module.bias']
            # model.load_state_dict(new_state_dict)

            # If model_FPN is data-parallel
            checkpoint['classifier1_state_dict']['module.classifier.weight'] = checkpoint['classifier1_state_dict'].pop('module.weight')
            checkpoint['classifier1_state_dict']['module.classifier.bias'] = checkpoint['classifier1_state_dict'].pop('module.bias')
            checkpoint['state_dict'].update(checkpoint['classifier1_state_dict'])
            model.load_state_dict(checkpoint['state_dict'])
            # classifier.load_state_dict(checkpoint['classifier1_state_dict'])
            # optimizer.load_state_dict(checkpoint['optimizer']) # comment when running cluster_in_seg.sh
...
def get_model_and_optimizer_loadpart(args, logger):
    # Init model 
    model = fpn.PanopticFPN(args)
    model = nn.DataParallel(model) # comment for same device
    model = model.cuda()
...
def get_model_and_optimizer_orig(args, logger):
    # Init model 
    model = fpn.PanopticFPN(args)
