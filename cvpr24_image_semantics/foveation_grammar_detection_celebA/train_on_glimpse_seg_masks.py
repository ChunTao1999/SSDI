...
model_FPN, optimizer = get_model_and_optimizer_loadpart(args, logger) # note dataparallel objects (fixed 2.13.2023)
logger.info('Adam optimizer is used.')
lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, gamma = 0.1, milestones=[40, 80])

# If initialized from middle
...
# =============================================================================
    ...
    train_loss_glimpse_mask     = 0.0 # 2.13.2023 - tao88
    test_loss                   = 0.0
    ...
    for i, (index, images, targets) in enumerate(train_loader): # target_type=['attr', 'bbox', 'pred_init_glimpse_locs', 'landmarks', 'masks']
        ...
        # if i == 0:
        #     save_image(translated_images[0], '/home/nano01/a/tao88/2.14/aligned_img.png')
...
# =============================================================================
        ...
        for g in range(config_3.num_glimpses):
            ...
            # if i == 0:
            #     save_image(glimpses_extracted_resized[0], "/home/nano01/a/tao88/2.14/img_0_g_{}.png".format(g))
            #     fig = plt.figure(figsize=(10,10))
            #     fig.add_subplot(1, 1, 1)
            #     plt.imshow(masks_extracted_resized[0].squeeze(0).cpu().numpy())
            #     plt.savefig('/home/nano01/a/tao88/2.14/img_0_gmask_{}.png'.format(g))
        ...
        optimizer.step()
        train_loss_glimpse_mask += loss_glimpse_mask.item()

        # pdb.set_trace()
        if (i+1) % 500 == 0:
            print("Current process: batch: {}/{}".format(i+1, len(train_loader)))
            # print("Loss: {}".format(train_loss_glimpse_mask))
    ...
    # Print losses
    print("Train Loss (glimse mask classification): {:.3f}\n".format(train_loss_glimpse_mask / (i+1)))
    log_dict['train_loss_glimpse_mask'].append(train_loss_glimpse_mask/(i+1))
    ...
    if (epoch+1) % 10 == 0: 
        torch.save({'epoch': epoch+1, 
                    ...
                    os.path.join('./glimpse_classification_models', 'checkpoint_{}.pth.tar'.format(epoch+1)))
