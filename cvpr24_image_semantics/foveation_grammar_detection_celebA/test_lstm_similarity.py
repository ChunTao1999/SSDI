            ...
            for img_id in range(16):
                if not config_3.landmark_shuffle:
                    if not config_3.use_corrupted_testset:
                        save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/correct/img_{}'.format(img_id)
                    else:
                        save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/corrupted_testset/img_{}'.format(img_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/corrupted/img_{}'.format(img_id)
...
# =============================================================================
        ...
        # sim_16_imgs = [[] for _ in range(16)]
        sim_batch_imgs = [[] for _ in range(translated_images.shape[0])]
        ...
        for g in range(config_3.num_glimpses):
            ...
            if g == 0:
                ...
                for sample_id in range(translated_images.shape[0]):
                    sim_batch_imgs[sample_id].append(torch.cosine_similarity(output_lstm[sample_id], g_semantics_pred[sample_id]).item())
        ...
        for sample_id in range(16):
            if not config_3.landmark_shuffle:
                if not config_3.use_corrupted_testset:
                    save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/correct/img_{}'.format(sample_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/corrupted_testset/img_{}'.format(sample_id)
            else:
                save_path = '/home/nano01/a/tao88/3.9_LSTM_sim/corrupted/img_{}'.format(sample_id)
            plot_curve(np.arange(1, 16), sim_batch_imgs[sample_id], 'Cossim btw. prediction and glimpse semantics', 'Glimpse iteration', 'Cossim', os.path.join(save_path, 'plot_dist.png'))

        # 3.8.2023 - tao88: try first plotting together the 16 curves of the 1st batch, blue for correct, red for corrupted
        x, y = [np.arange(1, 16) for _ in range(translated_images.shape[0])], sim_batch_imgs
        colors = ['b', 'r']
        ...
        plt.title('Cossim btw. prediction and glimpse semantics')
        plt.xlabel('Glimpse iteration')
        plt.ylabel('Cossim')
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.9_LSTM_sim/curves_all.png')

        # 3.9.2023 - tao88: plot an average distance curve for all correct images, and another for all corrupted
        sim_correct_imgs, sim_corrupted_imgs = [], []
        for j in range(translated_images.shape[0]):
            corrupt_label = corrupt_labels[j].item()
            if corrupt_label == 0:
                sim_correct_imgs.append(sim_batch_imgs[j])
            else:
                sim_corrupted_imgs.append(sim_batch_imgs[j])
        sim_correct_imgs = np.average(np.array(sim_correct_imgs), axis=0) # compute average curve for all correct images
        sim_corrupted_imgs = np.average(np.array(sim_corrupted_imgs), axis=0) # compute average curve for all corrupted images
        plt.figure()
        plt.plot(np.arange(1, 16), sim_correct_imgs, c='b', label=corruptness[0], linewidth=4)
        plt.plot(np.arange(1, 16), sim_corrupted_imgs, c='r', label=corruptness[1], linewidth=4)
        plt.title('Averaged cossim btw. prediction and glimpse semantics')
        plt.xlabel('Glimpse iteration')
        plt.ylabel('Cossim')
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.9_LSTM_sim/curves_together_avg.png')
        pdb.set_trace()
