...
with torch.no_grad():
    ...
    for batch_idx, sample in enumerate(test_loader):
        ...
        depth = sample['depth'].unsqueeze(-3).to(device) # half of depth images corrupted (bs, 1, 640, 480)
        ...
        if not LSTM_config.corruption_type == "puzzle_solving":
            ...
            pred_labels = (index_map_tensor[pred_labels] - 1).float().to(device) # (bs, 480, 640)
            ...
            # dist_batch_imgs_both[:, :-1] += dist_batch_imgs_back # (bs, num_patch)
            ...
            if batch_idx == 0:
                ...
                # dist_correct_imgs_for = torch.cat((dist_correct_imgs_for, dist_batch_imgs_for[corrupt_labels==0]), dim=0) # (#, num_patch-1)
                # dist_corrupted_imgs_for = torch.cat((dist_corrupted_imgs_for, dist_batch_imgs_for[corrupt_labels==1]), dim=0) # (#, num_patch-1)
                # dist_correct_imgs_back = torch.cat((dist_correct_imgs_back, dist_batch_imgs_back[corrupt_labels==0]), dim=0) # (#, num_patch-1)
                # dist_corrupted_imgs_back = torch.cat((dist_corrupted_imgs_back, dist_batch_imgs_back[corrupt_labels==1]), dim=0) # (#, num_patch-1)
                # dist_correct_imgs_both = torch.cat((dist_correct_imgs_both, dist_batch_imgs_both[corrupt_labels==0]), dim=0) # (#, num_patch)
                # dist_corrupted_imgs_both = torch.cat((dist_corrupted_imgs_both, dist_batch_imgs_both[corrupt_labels==1]), dim=0) # (#, num_patch)
    ...
    if not LSTM_config.corruption_type == "puzzle_solving":
        ...
        # # 5.6.2023 - for paper:
        # dist_correct_imgs_for, dist_corrupted_imgs_for = np.array(dist_correct_imgs_for.cpu()), np.array(dist_corrupted_imgs_for.cpu()) # both (#, num_patch-1)
        # dist_correct_imgs_back, dist_corrupted_imgs_back = np.array(dist_correct_imgs_back.cpu()), np.array(dist_corrupted_imgs_back.cpu()) # both (#, num_patch-1)
        # dist_correct_imgs_both, dist_corrupted_imgs_both = np.array(dist_correct_imgs_both.cpu()), np.array(dist_corrupted_imgs_both.cpu()) # both (#, num_patch)
        # 
        # # print data
        ...
        # 12, 160
        # plt.figure()
        # plt.hist(dist_correct_mean, bins=np.arange(5, 25.05, 0.05), alpha=0.7, color='b', label='correct')
        # plt.hist(dist_corrupted_mean, bins=np.arange(5, 25.05, 0.05), alpha=0.7, color='r', label='corrupted')
        # plt.xticks(np.arange(5, 27, 5))
