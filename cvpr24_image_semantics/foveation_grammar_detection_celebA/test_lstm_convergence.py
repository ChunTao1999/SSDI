            ...
            for img_id in range(16):
                if not config_3.landmark_shuffle:
                    if not config_3.use_corrupted_testset:
                        save_path = '/home/nano01/a/tao88/3.10_LSTM/correct/img_{}'.format(img_id)
                    else:
                        save_path = '/home/nano01/a/tao88/3.10_LSTM/corrupted_testset/img_{}'.format(img_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.10_LSTM/corrupted/img_{}'.format(img_id)
...
# =============================================================================
        ...
        convergence_batch_imgs = [15 for _ in range(translated_images.shape[0])]
        convergence_set = set()
        ...
        for g in range(config_3.num_glimpses):
            ...
            # 3.10.2023 - tao88: for images where background < 20%, record the current glimpse
            convergence = (g_semantics_pred[:,:,0] < 0.2).squeeze()
            for sample_id in range(translated_images.shape[0]):
                if sample_id in convergence_set:
                    continue
                if convergence[sample_id]:
                    convergence_batch_imgs[sample_id] = g
                    convergence_set.add(sample_id)
        ...
        for sample_id in range(16):
            if not config_3.landmark_shuffle:
                if not config_3.use_corrupted_testset:
                    save_path = '/home/nano01/a/tao88/3.10_LSTM/correct/img_{}'.format(sample_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.10_LSTM/corrupted_testset/img_{}'.format(sample_id)
            else:
                save_path = '/home/nano01/a/tao88/3.10_LSTM/corrupted/img_{}'.format(sample_id)
        ...
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.10_LSTM/curves_all.png')
        ...
        plt.ylabel('L2 distance')
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.10_LSTM/curves_together_avg.png')

        # pdb.set_trace() # check convergence_batch_imgs and corrupt_labels
        # 3.10.2023 - tao88
        # plot histograms
        correct_indices, wrong_indices = [k for k, x in enumerate(corrupt_labels) if x == 0], [k for k, x in enumerate(corrupt_labels) if x == 1]
        correct_convergences, wrong_convergences = [convergence_batch_imgs[i] for i in correct_indices], [convergence_batch_imgs[i] for i in wrong_indices]
        plt.figure()
        plt.hist(correct_convergences, range=(0, 15), bins=16, alpha=0.5, color='b', label='correct')
        plt.hist(wrong_convergences, range=(0, 15), bins=16, alpha=0.5, color='r', label='corrupted')
        plt.title('Histogram of correct and corrupted image convergence')
        plt.xlabel('Glimpe iteration')
        plt.ylabel('Frequency')
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.10_LSTM/hist_convergence.png')
        # number of matches and accuracy
        num_match= sum([1 for x,y in zip([1 if x > 4 else 0 for x in convergence_batch_imgs], corrupt_labels) if x == y])
        print("Number of samples recognized correctly:{}/{}".format(num_match, translated_images.shape[0]))
        pdb.set_trace()
