...
with torch.no_grad():
    for i, (index, images, targets) in enumerate(valid_loader): # target_type=['attr', 'bbox', 'pred_init_glimpse_locs', 'landmarks', 'masks']
            ...
            for img_id in range(16):
                if not config_3.landmark_shuffle:
                    if not config_3.use_corrupted_testset:
                        save_path = '/home/nano01/a/tao88/3.17_fivepatch/correct/img_{}'.format(img_id)
                    else:
                        save_path = '/home/nano01/a/tao88/3.17_fivepatch/corrupted_testset/img_{}'.format(img_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.17_fivepatch/corrupted/img_{}'.format(img_id)
        ...
        h_0, c_0 = model_4._init_hidden(translated_images.shape[0])
        ...
        for patch_id in range(5):
            ...
            # pass the patch semantics to the trained LSTM
            ...
            g_semantics_pred = torch.nn.functional.one_hot(lbl_predicted_flattened, num_classes=7).sum(dim=2) # (128, 1, 7)
            g_semantics_ratio_pred = (g_semantics_pred.float()) / num_pixels # (128, 1, 7)
        
            if patch_id == 0:
                output_lstm, (h, c) = model_4(g_semantics_ratio_pred, (h_0, c_0))
            else:
                ...
                output_lstm, (h, c) = model_4(g_semantics_ratio_pred, (h, c))
        
        # visualize the distance curves for indivisual images
        for sample_id in range(16):
            if not config_3.landmark_shuffle:
                if not config_3.use_corrupted_testset:
                    save_path = '/home/nano01/a/tao88/3.17_fivepatch/correct/img_{}'.format(sample_id)
                else:
                    save_path = '/home/nano01/a/tao88/3.17_fivepatch/corrupted_testset/img_{}'.format(sample_id)
            else:
                save_path = '/home/nano01/a/tao88/3.17_fivepatch/corrupted/img_{}'.format(sample_id)
        ...
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.17_fivepatch/curves_all.png')
        ...
        plt.ylabel('L2 distance')
        plt.legend()
        plt.savefig('/home/nano01/a/tao88/3.17_fivepatch/curves_together_avg.png')
