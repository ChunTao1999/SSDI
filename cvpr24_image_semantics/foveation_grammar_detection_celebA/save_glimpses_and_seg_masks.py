...
# =============================================================================
        ...
        for g in range(config_3.num_glimpses):
            ...
            for j in range(glimpses_extracted_resized_FPN.shape[0]): 
                save_image(glimpses_extracted_resized_FPN[j], "/home/nano01/a/tao88/celebA_raw/glimpse_imgs/trainval/img_{}_g_{}.jpg".format(index[j], g)) # save glimpses (3, 256, 256)
        ...
        # pdb.set_trace()
        # save the actions collected for the batch over all glimpses
        for j in range(translated_images.shape[0]): # for each image in the current batch
            torch.save(action_array[j], "/home/nano01/a/tao88/celebA_raw/glimpse_actions/trainval/img_{}_actions.pt".format(index[j]))
        if (i+1) % 100 == 0:
