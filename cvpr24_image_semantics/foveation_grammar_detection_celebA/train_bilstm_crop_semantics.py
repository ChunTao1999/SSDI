...
save_path = './lstm_models_on_five_patch_1e-4'
...
for epoch in range(0, config_3.epochs):
    ...
    for i, (indices, images, targets) in enumerate(train_loader):
        ...
        if config_3.data_parallel:
            h_0, c_0 = model_4.module._init_hidden(translated_images.shape[0]) # (1, 32, 7), (1, 128, 7)
        else:
            h_0, c_0 = model_4._init_hidden(translated_images.shape[0]) # (1, 128, 7), (1, 128, 7)
        loss_lstm = torch.tensor([0.0]).cuda()

        # Iterate through the patches
        for patch_id in range(5):
            g_semantics = torch.unsqueeze(mask_semantics_ratio[:, patch_id, :], 1) # (128, 1, 7)
            # we don't need the g_actions here
            if patch_id == 0:
                output_lstm, (h, c) = model_4(g_semantics, (h_0, c_0))
            else:
                loss_lstm += mse_loss(output_lstm, g_semantics)
                output_lstm, (h, c) = model_4(g_semantics, (h, c))
