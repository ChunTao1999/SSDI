...
def main(args, logger):
    ...
    with torch.no_grad():
        for batch_idx, (index, images) in enumerate(trainloader):
            ...
            out = model(images) # (256, 128, 64, 64)
            ...
            face_region_feat_indices = (lbl == 6).nonzero() # (number of matches * tensor_dimension), change 6 to other numbers if needed
            if batch_idx == 0:
            ...
            if not first_batch:
                ...
                for r in range(2):
                    for c in range(8):
                        ...
                        plt.imshow(empty_array[8*r+c])
                plt.savefig("/home/nano01/a/tao88/1.23/cluster_result_img_{}.png".format("0-16"))
                # fig = plt.figure(figsize=(8, 8))
