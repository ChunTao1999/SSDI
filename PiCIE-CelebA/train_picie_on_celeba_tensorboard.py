...
def parse_arguments():
    ...
    parser.add_argument('--landmark_shuffle', action='store_true', default=False) # can change here for the wrong img dataset
...
def train(args, logger, dataloader, model, classifier1, classifier2, criterion1, criterion2, optimizer, epoch):
    ...
    for i, (indice, input1, input2, label1, label2) in enumerate(dataloader):
        ...
        # tao88 - 1.16 debug, loss=nan issue
        # check in output12 and label12_processed whether there are nans and infs in these inputs
        # pdb.set_trace() - done
...
def main(args, logger):
    ...
    trainset, loss_weights = get_dataset(args, mode='train', inv_list=inv_list, eqv_list=eqv_list)
    # trainset.view = 2
    ...
    # pdb.set_trace()
    # tao88 - next
    # add some visualizations for random images in the trainset, before and after invariance and equivariance transform
    ...
    if not args.eval_only:
        ...
        for epoch in range(args.start_epoch, args.num_epoch):
            ...
            # 1.27 - tao88, tensorboard
            writer.add_scalar("Kmeans_loss/train_view_1", kmloss1, epoch)
            writer.add_scalar("Kmeans_loss/train_view_2", kmloss2, epoch)
            writer.add_scalar("CE_loss/train_total", train_cet, epoch)
            writer.add_scalar("CE_loss/train_within", cet_within, epoch)
            writer.add_scalar("CE_loss/train_across", cet_across, epoch)
...
if __name__=='__main__':
    ...
    # 1.27 - tao88, distinguish path between using finetuned models of different epochs
    args.save_model_path = os.path.join(args.save_root, args.comment, 'K_train={}_{}_finetuned_{}_epochs'.format(args.K_train, args.metric_train, 100))
    ...
    # 1.27 - tao88, Setup Tensorboard 
    writer = SummaryWriter(comment="seed_1_lr_1e-4_km_iter_20_num_clusters_{}".format(args.K_train)+"finetuned_model_coarse_100_epochs_"+"picie_epochs_20")
